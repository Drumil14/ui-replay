...
export function useRecorder(targetRef) {
  ...
  useEffect(() => {
    ...
    const onClick = (e) => {
      ...
      const labeledEl = e.target.closest('[data-label]');
      const interactiveEl =
        labeledEl ||
      ...
      const label =
        ...
        (interactiveEl?.tagName === 'INPUT' || interactiveEl?.tagName === 'TEXTAREA'
          ? interactiveEl.getAttribute('placeholder') || interactiveEl.name || 'input'
          : (interactiveEl?.textContent || '').trim().slice(0, 32) || interactiveEl?.tagName?.toLowerCase());
